...
def parse_input():
    ...
    g = {}
    for line in data[2:]:
        loc, leftright = line.split(" = ")
        g[loc] = leftright[1:-1].split(", ")
    return route, g
...
def part1():
    route, g = parse_input()
    loc = "AAA"
    for i, step in enumerate(itertools.cycle(route), start=1):
        loc = g[loc][0] if step == "L" else g[loc][1]
        if loc == "ZZZ":
            return i
...
def part2():
    route, g = parse_input()
    locs = [loc for loc in g if loc[-1] == "A"]
    distances = []
    for loc in locs:
        for i, step in enumerate(itertools.cycle(route), start=1):
            loc = g[loc][0] if step == "L" else g[loc][1]
