...
class Square:
    ...
    def next(self) -> Generator[Coord, None, None]:
        return (self.loc + offset for offset in self.offsets)
...
class Bar(Square):
    repr: str = "|"
    offsets: tuple[Coord, ...] = Coord(0, -1), Coord(0, 1)


class Dash(Square):
    repr: str = "-"
    offsets: tuple[Coord, ...] = Coord(1, 0), Coord(-1, 0)


class Ell(Square):
    repr: str = "L"
    offsets: tuple[Coord, ...] = Coord(0, -1), Coord(1, 0)
...
class Eff(Square):
    repr: str = "F"
    offsets: tuple[Coord, ...] = Coord(1, 0), Coord(0, 1)
...
class Map:
    def __init__(self, raw_map: str):
        self.rows = []
        for y, line in enumerate(raw_map.strip().splitlines()):
            self.rows.append([SQUARES[c](Coord(x, y)) for x, c in enumerate(line)])
        self.NX = len(self.rows[0])
        self.NY = len(self.rows)

    def __repr__(self):
        return "\n".join("".join(str(square) for square in row) for row in self.rows)

    def __len__(self):
        return len([square for square in self])
    ...
    def __getitem__(self, coord: Coord):
        if coord.x < 0 or coord.y < 0 or coord.x >= self.NX or coord.y >= self.NY:
            raise KeyError
        return self.rows[coord.y][coord.x]

    def __setitem__(self, coord: Coord, square: Square):
        if coord.x < 0 or coord.y < 0 or coord.x >= self.NX or coord.y >= self.NY:
            raise KeyError
        self.rows[coord.y][coord.x] = square
    ...
    def adjacents(self, square) -> Generator[Square, None, None]:
        for offset in (Coord(1, 0), Coord(-1, 0), Coord(0, 1), Coord(0, -1)):
            try:
                yield self[square.loc + offset]
            except KeyError:
                pass

    def find_connected_start(self) -> Generator[Square, None, None]:
        start = self.start
        adjacents = self.adjacents(start)
        yield from (
            adjacent for adjacent in adjacents if self.connected(start, adjacent)
        )

    def connected(self, square1, square2):
        return (square1.loc in (next for next in square2.next())) or (
            square2.loc in (next for next in square1.next())
        )

    def print_distances(self, distances):
        for row in self.rows:
            for square in row:
                print(distances.get(square) or square, end="")
            print()

    def print_painted(self, painted):
        for row in self.rows:
            for square in row:
                print("O" if square in painted else square, end="")
            print()
    ...
    def is_edge(self, square):
        if (
            square.loc.x == 0
            or square.loc.y == 0
            or square.loc.x == len(self.rows[0]) - 1
            or square.loc.y == len(self.rows) - 1
        ):
            return True
        else:
            return False
...
def find_distances(map) -> dict[Square, int]:
    q = deque()
    distances = {map.start: 0}
    q.extend((square, 1) for square in map.find_connected_start())
    while q:
        square, distance = q.popleft()
        if square in distances:
            continue
        distances[square] = distance
        q.extend((map[next], distance + 1) for next in square.next())
    return distances


def part1():
    map = Map(data)
    distances = find_distances(map)
    return max(distances.values())


def stretch_map(map):
    """
    Double the size of a map by adding empty rows and columns, maintaining
    connections between pipes by adding - and | pipes.
    """
    new_map = Map("\n".join("." * map.NX * 2 for _ in range(map.NY * 2)))
    for square in map:
        new_loc = 2 * square.loc
        new_map[2 * square.loc] = type(square)(2 * square.loc)
        if square.loc.x + 1 < map.NX and map.connected(
            square, map[square.loc + Coord(1, 0)]
        ):
            new_map[new_loc + Coord(1, 0)] = Dash(new_loc + Coord(1, 0))
        if square.loc.y + 1 < map.NY and map.connected(
            square, map[square.loc + Coord(0, 1)]
        ):
            new_map[new_loc + Coord(0, 1)] = Bar(new_loc + Coord(0, 1))
    return new_map
...
def part2():
    ...
    # Create gaps between unconnected pipes by doubling the size of the map.
    map = stretch_map(orig_map)

    # Paint this doubled map by expanding only into empty squares.
    painted = set()
    q = deque()

    # Start painting from every square on the edge that is not in loop (i.e.
    # is empty or a broken pipe)
    loop = set(find_distances(map))
    for square in map:
        if square not in loop and map.is_edge(square):
            q.append(square)

    while q:
        square = q.popleft()
        if square in painted:
            continue
        painted.add(square)
        q.extend(adjacent for adjacent in map.adjacents(square) if square not in loop)

    # Now for every square that is not in the loop in the original map, check if
    # the corresponding square in the doubled map was painted. If not, this is a
    # square of empty or broken pipe that is not reachable by painting the
    # original map, even if you're allowed to leak between adjacent unconnected
    # pipes. Such squares are the squares "inside" the loop.
    unpainted_orig = set()
    painted_locs = {square.loc for square in painted}
    loop_locs = {square.loc for square in loop}
    for orig_square in orig_map:
        square_loc = orig_square.loc * 2
        if square_loc not in loop_locs and square_loc not in painted_locs:
            unpainted_orig.add(orig_square)

    return len(unpainted_orig)
