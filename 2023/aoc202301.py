...
def process_line_1(line: str) -> int:
    for c in line:
        if c.isdigit():
            first = c
            break
...
def part1():
    return sum(process_line_1(line) for line in lines)
...
def find_number_as_word(s: str) -> int:
...
def check_position(s: str) -> int:
    if s[0].isdigit():
        return int(s[0])
    value = find_number_as_word(s)
    return value


def process_line_2(line: str) -> int:
    for i in range(len(line)):
        first = check_position(line[i:])
        if first:
            break
    for i in range(len(line) - 1, -1, -1):
        last = check_position(line[i:])
        if last:
            break
