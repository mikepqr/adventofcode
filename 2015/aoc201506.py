...
# two tuples to a list of all coordinates in box
def enclosed(a: Coord, b: Coord) -> list[Coord]:
    ax, ay = a
    bx, by = b
    return [(x, y) for x in range(ax, bx + 1) for y in range(ay, by + 1)]


# parse an instruction to verb, tuple, tuple
def parse_instruction(instruction: str) -> tuple[str, Coord, Coord]:
    if instruction.startswith("turn on"):
        ...
        verb = "toggle"
        i = 7
    tokens = instruction[i:].split()
    return (
        verb,
        tuple(int(x) for x in tokens[0].split(",")),
        tuple(int(x) for x in tokens[2].split(",")),
    )


N = 1000
instructions = [
    parse_instruction(unparsed_instruction)
...
def part1():
    ...
    for verb, a, b in instructions:
        if verb == "on":
            for x, y in enclosed(a, b):
                lights[x][y] = 1
        elif verb == "off":
            for x, y in enclosed(a, b):
                lights[x][y] = 0
